...
const Dashboard = () => {
  ...
  // --- FUNCIÓN DE CARGA (Ahora reutilizable) ---
  ...
  return (
    <div className="space-y-6">
          ...
          <div className="bg-gray-900 rounded-2xl p-6 text-white shadow-xl relative overflow-hidden">
            ...
            <div className="relative z-10">
              ...
              {viewMode === 'cashflow' ? (
                ...
                <div className="grid grid-cols-2 gap-4 text-sm"><div className="bg-white/10 p-3 rounded-lg"><p className="text-gray-400">Activos</p><p className="font-bold text-lg text-green-400">$35,400</p></div><div className="bg-white/10 p-3 rounded-lg"><p className="text-gray-400">Pasivos</p><p className="font-bold text-lg text-red-400">-$17,000</p></div></div>
              )}
            </div>
          </div>
        ...
        <div className="space-y-6">
          <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100 flex flex-col items-center text-center relative overflow-hidden">
            <div className="relative w-32 h-32 mb-4">
              <svg className="w-full h-full" viewBox="0 0 36 36"><path className="text-gray-100" d="M18 2.0845 a 15.9155 15.9155 0 0 1 0 31.831" fill="none" stroke="currentColor" strokeWidth="3" /><path className="text-green-500" strokeDasharray="85, 100" d="M18 2.0845 a 15.9155 15.9155 0 0 1 0 31.831" fill="none" stroke="currentColor" strokeWidth="3" strokeLinecap="round" /></svg>
              <div className="absolute inset-0 flex flex-col items-center justify-center"><span className="text-3xl font-bold text-gray-800">85</span><span className="text-[10px] uppercase font-bold text-green-600 tracking-wider">Excelente</span></div>
            </div>
      ...
      {/* 👇 EL MODAL INVISIBLE (Solo se muestra cuando isModalOpen es true) */}
      <TransactionModal 
        isOpen={isModalOpen} 
        onClose={() => setIsModalOpen(false)}
        type={modalType}
        onSuccess={fetchData} // Le pasamos la función para que recargue la lista
      />
    </div>
  );
};
